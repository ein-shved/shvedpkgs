{ pkgs, config }:
let
    tabwidth = builtins.toString (config.vim.tabwidth or 4);
    linewidth = config.vim.linewidth or 80;
...
"Highlight next after linewidth column
    set colorcolumn=${builtins.toString (linewidth + 1)}
...
"Make vim to complete commands in the bach-like way:
...
"LSP
    ...
    let g:lsp_settings = {  'clangd': {'cmd': ['clangd', '-j=4']}}
