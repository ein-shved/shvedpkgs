...
let
  cfg = config.programs.swaylock-upd;
in
{
  meta.maintainers = [ lib.hm.maintainers.rcerc ];

  options.programs.swaylock-upd = {
    enable = lib.mkOption {
      type = lib.types.bool;
    ...
    settings = lib.mkOption {
      type =
        ...
        in
        attrsOf (oneOf (leaf ++ [ (listOf (oneOf leaf)) ]));
      default = { };
      description = ''
        ...
        let
          mkLeafRecord =
            ...
            if v == false then
              ...
              + "\n";
          mkRecord = (n: v: lib.map (mkLeafRecord n) (lib.toList v));
        in
        lib.concatStrings (lib.flatten (lib.mapAttrsToList mkRecord cfg.settings));
    };
  };
}
